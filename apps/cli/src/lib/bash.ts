...
export async function ffmpeg_compress_mp4_gpu(
  ...
  const encoders = {
    ...
    mac: "-c:v h264_videotoolbox -q:v 60",
  ...
  // 3. Aplica a inicialização correta para VA-API no Host (FFmpeg 7+)
  if (isLinux && (gpu === "amd" || gpu === "intel")) {
    // Cria o dispositivo de hardware explicitamente e o vincula aos filtros
    hwaccel =
  ...
  // 4. Monta e executa o comando final
